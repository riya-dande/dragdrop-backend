...
function mapResponse(response: any) {
  const values = Object.fromEntries(
    (response.values ?? []).map((item: any) => [item.field?.id ?? item.dataControlId, item.value])
  );
...
async function getFormWithFields(id: string) {
...
function buildFieldData(field: FieldInput, index: number) {
  return {
    controlKey: String(field.controlKey ?? field.id ?? `field_${index + 1}`),
    type: normalizeFieldType(field.type) as any,
    label: field.label?.trim() || `Field ${index + 1}`,
    placeholder: field.placeholder ?? "",
    required: field.required ?? false,
    dropdownItems: field.dropdownItems as any,
    sortOrder: index,
  };
}

function buildFieldCreates(fields: FieldInput[]) {
  return fields.map((field, index) => buildFieldData(field, index));
}

async function buildRecordPayload(formId: string, values: Record<string, unknown>) {
...
frontendRouter.get("/health", (_req: Request, res: Response) => {
  res.status(200).json({
    status: "ok",
  });
});
...
frontendRouter.get("/apps", async (req: Request, res: Response) => {
  try {
...
frontendRouter.post("/apps", async (req: Request, res: Response) => {
  try {
...
frontendRouter.put("/apps/:id", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
...
frontendRouter.delete("/apps/:id", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const appId = paramValue(req.params.id);
...
frontendRouter.post("/forms", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
...
frontendRouter.get("/forms/current", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const where = user && user.role !== "ADMIN" ? { createdBy: user.id } : undefined;
...
frontendRouter.get("/forms", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const where = user && user.role !== "ADMIN" ? { createdBy: user.id } : undefined;
    const forms = await prisma.form.findMany({
...
frontendRouter.get("/forms/:id", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
...
frontendRouter.put("/forms/:id", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    ...
    const existingFieldsBySignature = new Map(
      existingForm.fields.map(field => [`${String(field.type).toLowerCase()}::${field.label.trim().toLowerCase()}`, field])
    );
    ...
    const updatedForm = await prisma.$transaction(async transaction => {
      ...
      for (const [index, field] of fields.entries()) {
        ...
        const existingField =
          existingFieldsById.get(fieldId) ??
...
frontendRouter.delete("/forms/:id", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const formId = paramValue(req.params.id);
...
frontendRouter.post("/forms/:formId/responses", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
...
frontendRouter.get("/forms/:formId/responses", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const formId = paramValue(req.params.formId);
...
frontendRouter.get("/responses", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
...
frontendRouter.put("/responses/:recordId", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
...
frontendRouter.delete("/responses/:recordId", async (req: Request, res: Response) => {
  try {
    const user = getAuthUser(req);
    const recordId = paramValue(req.params.recordId);
