...
import { createForm, getAllForms, getFormById, getFormsByAppId, } from "./form.service.js";
...
export async function getAllFormsController(req, res) {
    try {
...
export async function getFormByIdController(req, res) {
    try {
        const result = await getFormById(req.params.id, getAuthUser(req));
        if (!result) {
...
export async function getFormsByAppIdController(req, res) {
    try {
        const result = await getFormsByAppId(req.params.appId, getAuthUser(req));
        return res.status(200).json({
            error: false,
            data: result,
            message: "forms found successfully",
